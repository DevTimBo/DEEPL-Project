...
# Main Class to start the Application/GUI
...
class Ui(QtWidgets.QDialog):
    ...
    def image_analyzer(self, image, image_path):
        ...
        if self.noise_walk_checkbox.isChecked():
            ...
            #resized_image = self.keras_preprocess(resized_image)
    ...
    def grad_cam_analyze(self, image_path):
        ...
        print("---GradCAM %s seconds ---" % (time.time() - start_time))
    ...
    def lime_analyzer(self, image, samples, features, positive_only, hide_rest, mask_only, min_weight):
        ...
        print("---LIME %s seconds ---" % (time.time() - start_time))
    ...
    def mcDropout(self, image, mcd_samples, mcd_dropoutrate, mcd_apply_skip, mcd_layers):
        ...
        print("---MCD %s seconds ---" % (time.time() - start_time))
...
def create_mcd_image(text1, text2, pred1, float_pred1, pred2, float_pred2, pred3, float_pred3, pred4, float_pred4,
    ...
    # Draw a linePlot
    # blue is red because of rgb to bgr 
    draw.line((x1, y1, x1 + int(width / 100 * float_pred1), y1), fill=("blue"), width=int(height / 10))
    draw.line((x2, y2, x2 + int(width / 100 * float_pred2), y2), fill=("blue"), width=int(height / 10))
    draw.line((x3, y3, x3 + int(width / 100 * float_pred3), y3), fill=("blue"), width=int(height / 10))
    draw.line((x4, y4, x4 + int(width / 100 * float_pred4), y4), fill=("blue"), width=int(height / 10))
    draw.line((x5, y5, x5 + int(width / 100 * float_pred5), y5), fill=("blue"), width=int(height / 10))
